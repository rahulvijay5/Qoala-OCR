...
const UpdateForm = () => {
  ...
  const handleUpdateData = async () => {
    ...
    const response = await axios.put(`http://localhost:3001/api/ocr-record/${recordId}`, {
    ...
    <div className='my-6 flex flex-col justify-center items-center rounded-md border-2  p-3'>
      ...
      <form className='mt-8 mb-2 rounded-md'>
        ...
        <div className='flex items-center justify-between'>
            ...
            <option value="result.identification_number">Identification Number</option>
        ...
        {notFillDetails && (
            <div className="text-center text-md my-2 p-1 rounded-md bg-red-500/50">Fill all the details first.</div>
          )}

        <div className='flex justify-center'>
            <button  type="button" className="outline p-1 rounded-md mt-6 w-1/2 hover:bg-black font-semibold hover:text-white" onClick={handleUpdateData}>
            Update Data
            </button>
        </div>
      </form>
